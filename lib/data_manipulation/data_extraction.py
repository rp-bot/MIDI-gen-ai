...
FOLDER_PATH = os.path.join(
    os.getcwd(), "Ultimate-MIDI-Scraper", "data", "MIDIdata", "rock")

DB = os.path.join(
    os.getcwd(), "Ultimate-MIDI-Scraper", "data", "db", "AllMIDI.sqlite3")

CURRENT_PROJ = os.path.join(os.getcwd(), "data_manipulation")
...
def determine_max_num_songs(percentile):
    # List all folders
...
def create_data_set(dest_directory: str):
    ...
    max_num_songs = determine_max_num_songs(28)
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(large_MIDI_language_list, columns=['rock'])
    df.to_parquet(os.path.join(CURRENT_PROJ, "rock_extracted_midi.parquet"))
